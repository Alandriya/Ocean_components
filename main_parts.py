...
if __name__ == '__main__':
    ...
    #         start_index = days_delta1 + days_delta2 + days_delta3 + days_delta4
    #
    #     flux = np.load(files_path_prefix + f'Fluxes/FLUX_{start_year}-{end_year}_grouped.npy')
    #     flux = flux.transpose()
    #     flux = flux.reshape((-1, height, width))
    #     pressure = np.load(files_path_prefix + f'Pressure/PRESS_{start_year}-{end_year}_grouped.npy')
    #     pressure = pressure.transpose()
    #     pressure = pressure.reshape((-1, height, width))
    #
    #     arr = np.zeros((flux.shape[0], height, width, 2))
    #     arr[:, :, :, 0] = flux
    #     arr[:, :, :, 1] = pressure
    #     arr[:, np.logical_not(mask), :] = np.nan
    #
    #     count_semi_2d_AB(files_path_prefix, arr, 0, arr.shape[0], mask, quantiles_amount=15,
    #                      start_index=start_index, path='Components/flux-pressure/')
    #
    #     collect_estimates(files_path_prefix, 'Components/flux-pressure/Semi_2d/', ['A', 'B'],
    #                       1 + start_index, arr.shape[0] + start_index, )
    #
    # for start_year in [2019]:
    #     if start_year == 2019:
    #         end_year = 2026
    ...
    #
    #
    #     collect_estimates(files_path_prefix, 'Components/sensible-latent/Semi_2d/', ['A', 'B'],
    #                       1 + start_index, arr.shape[0] + start_index,)
    # ----------------------------------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------------------------------
    # # Mean year collect A and B semiparametric
    # str_types = 'sensible-latent'
    # # str_types = 'flux-pressure'
    # mean_a = np.zeros((364, 161, 181, 2))
    # mean_b = np.zeros((364, 161, 181, 4))
    # for start_year in [1979, 1989, 1999, 2009, 2019]:
    #     end_year = start_year + 10
    #     if start_year == 1979:
    #         first = 1
    ...
    #
    #     print(f'Parsing {start_year}-{end_year}')
    #     a = np.load(files_path_prefix + f'Components/{str_types}/Semi_2d/A_{first}-{end}.npy')
    #     b = np.load(files_path_prefix + f'Components/{str_types}/Semi_2d/B_{first}-{end}.npy')
    #     for j in range(end_year-start_year):
    #         time_start = (datetime.datetime(year=start_year + j, month=1, day=2) - datetime.datetime(year=start_year, month=1,
    #                                                                                        day=1)).days
    #         time_end = time_start + 364
    #         # print(time_start)
    #         # print(time_end)
    #         mean_a += a[time_start:time_end]
    #         mean_b += b[time_start:time_end]
    #
    # mean_a /= (2024 - 1979)
    # mean_b /= (2024 - 1979)
    #
    # np.save(files_path_prefix + f'Mean_year/mean_{str_types}_A.npy', mean_a)
    # np.save(files_path_prefix + f'Mean_year/mean_{str_types}_B.npy', mean_b)
    # ----------------------------------------------------------------------------------------------
    # # Mean year
    # mean_year = np.zeros((365, 161, 181))
    # for start_year in [1979, 1989, 1999, 2009, 2019]:
    ...
    #     if end_year == 2026:
    #         end_year = 2024
    #         delta = (datetime.datetime(year=end_year, month=1, day=1) - datetime.datetime(year=start_year, month=1,
    #                                                                                        day=1)).days
    #         sensible = sensible[:delta]
    #         latent = latent[:delta]
    #         press = press[:delta]
    #
    #     plot_hist(files_path_prefix, 'sensible' + f'_{start_year}-{end_year}', sensible)
    #     plot_hist(files_path_prefix, 'latent' + f'_{start_year}-{end_year}', latent)
    #
    #     for j in range(end_year-start_year):
    #         time_start = (datetime.datetime(year=start_year + j, month=1, day=1) - datetime.datetime(year=start_year, month=1,
    #                                                                                        day=1)).days
    ...
    # raise ValueError
    # ----------------------------------------------------------------------------------------------
    data1_array = np.zeros((29141, days_delta1 + days_delta2 + days_delta3 + days_delta4 + days_delta8))
    shift = 0
    for start_year in [1979, 1989, 1999, 2009, 2019]:
        end_year = start_year + 10
        if start_year == 2019:
            end_year = 2026
        tmp = np.load(files_path_prefix + f'DATA/Fluxes/sensible_grouped_{start_year}-{end_year}.npy')
        data1_array[:, shift:shift + tmp.shape[1]] = tmp[:, 0:min(tmp.shape[1], data1_array.shape[1]-shift)]
        shift += tmp.shape[1]
        print(shift)
    np.save(files_path_prefix + f'DATA/Fluxes/sensible_grouped_1979-2024.npy', data1_array)

    data2_array = np.zeros((29141, days_delta1 + days_delta2 + days_delta3 + days_delta4 + days_delta8))
    shift = 0
    for start_year in [1979, 1989, 1999, 2009, 2019]:
        end_year = start_year + 10
        if start_year == 2019:
            end_year = 2026
        tmp = np.load(files_path_prefix + f'DATA/Fluxes/latent_grouped_{start_year}-{end_year}.npy')
        data2_array[:, shift:shift + tmp.shape[1]] = tmp[:, 0:min(tmp.shape[1], data2_array.shape[1]-shift)]
        shift += tmp.shape[1]
        print(shift)
    np.save(files_path_prefix + f'DATA/Fluxes/latent_grouped_1979-2024.npy', data2_array)

    a_array = np.zeros((17106, 161, 181, 2,))
    b_array = np.zeros((17106, 161, 181, 4,))
    ...
    for start_year in [1979, 1989, 1999, 2009, 2019]:
        ...
        a = np.load(files_path_prefix + f'Components/{str_types}/Semi_2d/A_{coef_start}-{coef_end}.npy')
        print(a.shape)
        b = np.load(files_path_prefix + f'Components/{str_types}/Semi_2d/B_{coef_start}-{coef_end}.npy')
        a_array[shift:shift + a.shape[0]] = a[0:min(a.shape[0], a_array.shape[0] - shift)]
        b_array[shift:shift + b.shape[0]] = b[0:min(b.shape[0], b_array.shape[0] - shift)]
        shift += a.shape[0]
        print(shift)
    np.save(files_path_prefix + f'Components/{str_types}/Semi_2d/A_1-17106.npy', a_array)
    np.save(files_path_prefix + f'Components/{str_types}/Semi_2d/B_1-17106.npy', b_array)
    # ----------------------------------------------------------------------------------------------
    start_year = 19792024
